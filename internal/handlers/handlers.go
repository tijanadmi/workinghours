...
// AdminReservationsCalendarByDayType displays the reservation calendar
func (m *Repository) AdminReservationsCalendarByDayType(w http.ResponseWriter, r *http.Request) {
	// assume that there is no month/year specified
	now := time.Now()
	...
	/**** get workingDayTypeList for the list Begin***/

	workingDayTypeList, err := m.DB.GetWorkingDayTypes()
	...
	/**** get workingDayTypeList for the list End***/
	data["workingDayTypeList"] = workingDayTypeList
	...
	for _, x := range employee {
		...
		// get all the reservations for the current employee for day shift
		reservationsDay, err := m.DB.GetReservationTypeForEmpByDate(x.ID, firstOfMonth, lastOfMonth)
		if err != nil {
			helpers.ServerError(w, err)
			return
		}
	...
	render.Template(w, r, "admin-reservations-calendar-day-type.page.tmpl", &models.TemplateData{
		StringMap: stringMap,
		Data:      data,
		IntMap:    intMap,
	})
...
func (m *Repository) AdminShowDashboardCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range employee {
		...
		reservationMap := make(map[string]int)
		blockMap := make(map[string]int)
		dayTypeblockMap := make(map[string]string)
		...
		for d := firstOfMonth; !d.After(lastOfMonth); d = d.AddDate(0, 0, 1) {
			reservationMap[d.Format("2006-01-2")] = 0
			blockMap[d.Format("2006-01-2")] = 0
			dayTypeblockMap[d.Format("2006-01-2")] = ""
			nightblockMap[d.Format("2006-01-2")] = 0
		}
		...
		for _, y := range reservationsDay {
			...
			// it's a block
			blockMap[y.StartDate.Format("2006-01-2")] = y.WdTypeId
			dayTypeblockMap[y.StartDate.Format("2006-01-2")] = y.WorkingDayType.Code

			fmt.Printf("EMP_ID=%d, Blocked value=%s For day=%s ", x.ID, dayTypeblockMap[y.StartDate.Format("2006-01-2")], y.StartDate.Format("2006-01-2"))
			fmt.Println()
		...
		data[fmt.Sprintf("reservation_map_%d", x.ID)] = reservationMap
		data[fmt.Sprintf("block_map_%d", x.ID)] = blockMap
		data[fmt.Sprintf("day_type_block_map_%d", x.ID)] = dayTypeblockMap

		data[fmt.Sprintf("night_block_map_%d", x.ID)] = nightblockMap

		m.App.Session.Put(r.Context(), fmt.Sprintf("block_map_%d", x.ID), blockMap)
		m.App.Session.Put(r.Context(), fmt.Sprintf("day_type_block_map_%d", x.ID), dayTypeblockMap)
